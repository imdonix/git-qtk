import { WILDCARD, wrapFunc } from "./utils.js";
...
export async function post() {
  ...
  const limited = limit(selected, this.limit);

  return limited;
}

function order(input, order, funs, reductors) {
  if (order != null) {
    const [exp, pre] = order;
    const isReductor = exp.indexOf(`${WILDCARD.SP}r`) >= 0;
    const p = isReductor
      ? wrapFunc(exp.toString(), ["__o", "__f", "__r", "__tmp"])
      : wrapFunc(exp.toString(), ["__o", "__f"]);

    input.sort((a, b) =>
      pre(
        isReductor ? p(a, funs, reductors, null) : p(a, funs),
        isReductor ? p(b, funs, reductors, null) : p(b, funs),
      )
        ? 1
        : -1,
    );
...
function limit(input, lim) {
  if (lim != null && Array.isArray(input)) {
    return input.slice(0, lim);
  }
...
function select(input, select, group, funs, reductors, fields) {
  ...
  if (group != null) {
    ...
    for (const r of reduced) {
      redfun[r[0][0]] = wrapFunc(r[0][0].toString(), [
        "__o",
        "__f",
        "__r",
        "__tmp",
  ...
  for (const se of select) {
    preds[se[0]] = wrapFunc(se[0], ["__o", "__f"]);
  }
