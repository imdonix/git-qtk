...
import { WILDCARD, OPERATOR } from "./utils.js";

const JOIN =
  /([a-zA-Z][a-zA-Z1-9._]*\.[a-zA-Z][a-zA-Z1-9._]*)\s*==\s*([a-zA-Z][a-zA-Z1-9._]*\.[a-zA-Z][a-zA-Z1-9._]*)/g;
...
export function parseFrom(query) {
  ...
  const insert = (key, val) => {
    if (query.from.has(key)) {
      throw new Error(
        `A model with this name (${key}) is already in the query.`,
      );
    } else {
...
export function parseLimit(query) {
  if (!query.yaml.hasOwnProperty("limit")) {
    ...
    const resolved = resolveParameters(query, query.yaml.limit.toString());
    query.limit = parseInt(resolved);

    if (Number.isNaN(query.limit)) {
      throw new Error(`Limit must be an positive integer, got: ${resolved}`);
    }
...
export function parseOrder(query) {
  if (!query.yaml.hasOwnProperty("order") || query.yaml["order"] == null) {
    ...
    expression = insertFieldBinding(query, expression);
    expression = insertFunctionBinding(query, expression);
    expression = insertReductorBinding(query, expression);
...
function insertReductorBinding(query, expression) {
  for (const [key, _] of Object.entries(query.reductors)) {
    expression = expression.replace(
      new RegExp(`${key}\\(`, "g"),
      `${WILDCARD.SP}r.${key}(${WILDCARD.SP}tmp,`,
    );
  }
...
function resolveParameters(query, expression) {
  ...
  for (const match of expression.matchAll(VAR)) {
    const key = match[1];
    if (key in query.query.params) {
      filled = filled.replaceAll(`{${key}}`, query.query.params[key]);
    } else {
      throw new Error(
        `Script uses '${key}' parameter but its not given in command line [param=value]`,
      );
    }
  }

  if (query.query.args && query.query.args.length > 0) {
    for (let i = 0; i < query.query.args.length; i++) {
      filled = filled.replaceAll(`$${i}`, query.query.args[i]);
    }
  }
...
function decompose(str) {
  ...
  for (let i = 0; i < str.length; i++) {
    if (str[i] == "&") {
      ...
      if (depth == 0) {
        if (and == 2) {
          tmp.push(expression.slice(0, -1)); // remove the first &
          and = 0;
          expression = "";
        }
      } else {
  ...
  return tmp.map((str) => str.trim()).filter((s) => s.length > 0);
}
