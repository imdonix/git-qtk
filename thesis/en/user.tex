...
\begin{itemize}
	...
	\item Multiple model can be added by separating them with the \citebf{';'} delimiter.
...
\begin{itemize}
	\item A single statement can be added with the \citebf{'{model}.{field}'} syntax.
	\item Multiple statement can be selected by separating them with the \citebf{';'} delimiter.
	\item Each statement will be interpreted and evaluated as JavaScript.
	\item The \citebf{\$} symbol is a wildcard to select all fields. 
\end{itemize}
...
\begin{itemize}
	...
	\item The record fields can be referenced as \citebf{'{model}.{field}'}
\end{itemize}
...
\citebf{{model}.{field} == {model}.{field}}\newline

In this case the following rules applies:

\begin{itemize}
	\item If only one of the field is a key for the model then its a \citebf{Right or Left Join}
	\item If both of them are key then its a \citebf{Inner Join}
...
\begin{itemize}
	\item Descending order: \citebf{DESC}
	\item Ascending order: \citebf{ASC}
...
The following rules applies:
\begin{itemize}
	\item A single field should be added as \citebf{'{model}.{field}'}
\end{itemize}
